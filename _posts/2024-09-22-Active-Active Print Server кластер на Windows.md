...
Первично нужно будет потратить время на детальный анализ всех принтеров которые подключены пользователям. В моем случае оказалось что все кейсы можно покрыть довольно мылым количеством драйверов. Их необходимо расположить на сетевой шаре, доступной для чтения группе `Domain Computers`.
...
## Результат
После выполнения описанных инструкций на наших серверах будут установлены необходимые принтеры, чтобы получать задания печати, а так же у нас будет возможность переключить поток этих заданий на другой сервер печати изменяя DNS Alias. Возможно что пользователям уже установившим соединение с сервером придётся совершить перезагрузку или перелогиниться, чтобы доступ к печати был восстановлен. Это связано с Kerberos тикетом для работы с сервером и может быть решено их сбросом, либо перезагрузкой.
